...
def tag_batch(client, items):
    """Tag multiple items in a single API call."""
    lines = []
...
JSON array only, no markdown."""
    ...
    try:
        ...
        return [{{"i": i, "error": str(e)}} for i in range(len(items))]
...
def tag_item(client, item):
    ...
    if results and "error" not in results[0]:
        ...
        item.update({"clean_title": "", "topic_tags": "", "action_type_ai": "",
                     "geography": "", "summary": "", "tag_status": f"error: {results[0].get('error','unknown')}"})
    return item
    ...
    if not truly_new:
        ...
        for batch_start in range(0, len(to_tag), BATCH_SIZE):
            ...
            for j, item in enumerate(batch):
                p = next((r for r in results if isinstance(r, dict) and r.get("i") == j), None)
